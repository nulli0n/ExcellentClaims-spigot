...
@NullMarked
public class EditorPriorityDialog extends Dialog<LandClaim> {

    private static final TextLocale TITLE = LangEntry.builder("Lands.UI.Editor.Dialog.Priority.Title")
        .text(title("Claim", "Priority"));

    private static final DialogElementLocale BODY = LangEntry.builder("Lands.UI.Editor.Dialog.Priority.Body")
        .dialogElement(400,
            "Enter a new priority value for the claim."
        );

    private static final TextLocale INPUT_PRIORITY = LangEntry.builder("Lands.UI.Editor.Dialog.Priority.Input.Priority")
        .text("Priority");
    ...
    public EditorPriorityDialog(LandEditorService editor, MessageDispatcher dispatcher) {
        super();
        this.editor = editor;
