...
@NullMarked
public class EditorNameDialog extends Dialog<LandClaim> {

    private static final TextLocale TITLE = LangEntry.builder("Lands.UI.Editor.Dialog.Name.Title")
        .text(title("Claim", "Name"));

    private static final DialogElementLocale BODY = LangEntry.builder("Lands.UI.Editor.Dialog.Name.Body")
        .dialogElement(400,
            "Enter a new name for the claim."
        );

    private static final TextLocale INPUT_NAME = LangEntry.builder("Lands.UI.Editor.Dialog.Name.Input.Name")
        .text("Name");
    ...
    public EditorNameDialog(LandSettings settings, LandEditorService editor, MessageDispatcher dispatcher) {
        super();
            ...
            .base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.text(JSON_NAME, INPUT_NAME)
                        ...
                        .maxLength(this.settings.getLandNameMaxLength() * 2) // x2 for space for color codes
                        .build()
                )
                .build()
            )
