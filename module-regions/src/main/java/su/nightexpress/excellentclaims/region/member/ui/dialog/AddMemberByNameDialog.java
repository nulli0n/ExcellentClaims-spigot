...
@NullMarked
public class AddMemberByNameDialog extends Dialog<RegionClaim> {
    ...
    @Override
    public WrappedDialog create(Player player, RegionClaim context) {
        return Dialogs.create(builder -> builder
            ...
            .handleResponse(DialogActions.CONFIRM, (viewer, identifier, nbtHolder) -> {
                ...
                // TODO this.controller.onSelectMemberToAdd(player, context.claim(), playerId);
                viewer.callback();
            })
            .build()
        );
