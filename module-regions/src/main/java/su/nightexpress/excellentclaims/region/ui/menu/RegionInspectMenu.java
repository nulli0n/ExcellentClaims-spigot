...
@NullMarked
public class RegionInspectMenu extends AbstractObjectMenu<InspectContext> {

    private static final String DEFAULT_TITLE = "[%s] Regions"
    ...
    public RegionInspectMenu(ClaimPlugin plugin,
    ...
    protected void onLoad(FileConfig config) {
        ...
        List<String> regionLoreAdmin = config.getOrSet("Region.Lore.Admin", ConfigCodecs.STRING_LIST, Lists.newList(
            ...
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("ID: ") + RegionsPlaceholders.REGION_ID),
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("World: ") + RegionsPlaceholders.REGION_WORLD),
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("Priority: ") + RegionsPlaceholders.REGION_PRIORITY),
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("Members: ") + RegionsPlaceholders.REGION_MEMBERS),
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("Chunks: ") + RegionsPlaceholders.REGION_SIZE),
            CommonPlaceholders.EMPTY_IF_ABOVE,
            TagWrappers.RED.wrap("→ " + TagWrappers.UNDERLINED.wrap("Click to edit"))
        ));

        List<String> regionLoreViewer = config.getOrSet("Region.Lore.Viewer", ConfigCodecs.STRING_LIST, Lists.newList(
            RegionsPlaceholders.REGION_DESCRIPTION,
            CommonPlaceholders.EMPTY_IF_ABOVE,
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("Rank: ") + ClaimsPlaceholders.RANK_NAME),
            TagWrappers.GOLD.wrap("» " + TagWrappers.GRAY.wrap("World: ") + RegionsPlaceholders.REGION_WORLD),
        ...
        this.regionItemPopulator = ItemPopulator.builder(RegionClaim.class)
            ...
            .itemProvider((context, claim) -> {
                ...
                return claim.getIcon()
                    ...
                    .setLore(isAdmin ? regionLoreAdmin : regionLoreViewer)
                    .hideAllComponents()
                    .replace(ctx -> {
    ...
    public void onPrepare(ViewerContext context, InventoryView view, Inventory inventory, List<MenuItem> items) {
        InspectContext inspectContext = this.getObject(context);
        List<RegionClaim> claims = inspectContext.regions();
