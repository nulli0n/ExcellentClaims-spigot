...
public class SetNameCommand implements CommandExtension {
    ...
    private final RegionEditorService      editorService;
    private final MessageDispatcher        dispatcher;

    public SetNameCommand(RegionCommandResolver resolver, RegionCommandSuggestions suggestions,
                          RegionEditorService editorService, MessageDispatcher dispatcher) {
    ...
    public ExecutableNode createCommand() {
        return Commands.literal("rename", builder -> builder
            .playerOnly()
            .description(RegionLang.COMMAND_REGION_SET_NAME_DESC)
            .permission(RegionPerms.COMMAND_SET_NAME)
            .withArguments(
