...
@NullMarked
public class EditorNameDialog extends Dialog<RegionClaim> {

    private static final TextLocale TITLE = LangEntry.builder("Regions.UI.Editor.Dialog.Name.Title")
        .text(title("Region", "Name"));

    private static final DialogElementLocale BODY = LangEntry.builder("Regions.UI.Editor.Dialog.Name.Body")
        .dialogElement(400,
            "Enter a new name for the region."
        );

    private static final TextLocale INPUT_NAME = LangEntry.builder("Regions.UI.Editor.Dialog.Name.Input.Name")
        .text("Name");
    ...
    public EditorNameDialog(RegionSettings settings, RegionEditorService editor, MessageDispatcher dispatcher) {
        super();
            ...
            .base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.text(JSON_NAME, INPUT_NAME)
                        ...
                        .maxLength(this.settings.getRegionNameMaxLength() * 2) // x2 for space for color codes
                        .build()
                )
                .build()
            )
