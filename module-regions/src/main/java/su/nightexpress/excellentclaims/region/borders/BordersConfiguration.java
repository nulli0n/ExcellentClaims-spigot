...
import su.nightexpress.excellentclaims.region.borders.command.BordersCommand;
...
import su.nightexpress.excellentclaims.region.borders.settings.BorderSettingsConfiguration;

@NullMarked
public final class BordersConfiguration {

    private static final Identifier ID = Identifier.of("regions_bounds");
    ...
    public static void configure(RegionsModule module, DependencyContainer container) {
        ...
        if (highlightAPI == null) {
            logger.info("Highlight API is not available, Regions Bounds Highlight feature will be unavailable.");
            return;
        }
